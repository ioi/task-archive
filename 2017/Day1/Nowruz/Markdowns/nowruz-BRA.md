...
Faltam poucos dias para o Nowruz (ano novo persa) e o avô convidou a sua família para o seu jardim. Existem $k$ crianças entre os seus convidados. Para tornar o encontro mais divertido, o avô vai organizar um jogo de esconde esconde.
...
Será dado um mapa do jardim como entrada. A sua tarefa é ajudar o avô a criar um labirinto no qual muitas crianças podem se esconder.

## Detalhes de Implementação

Esta é uma tarefa de "apenas saída" com pontuações parciais. Serão dados dez arquivos de entrada, cada um descrevendo o jardim do avô. Para cada um dos arquivos de entrada deverá submeter um arquivo de saída com um mapa do labirinto. Para cada arquivo de saída irá receber pontos de acordo com o número de crianças que conseguem se esconder no labirinto.

Não é suposto que submeta código-fonte para este problema.

## Formato de entrada

Cada arquivo de entrada descreve uma matriz representando um jardim e indica o número de crianças convidadas pelo avô. O formato é o seguinte:
...
Um arquivo de saída é considerado *válido* se respeitar as seguintes condições:
...
Se o output de um caso de saída não for válido, a sua pontuação para esse caso será 0. Caso contrário, a pontuação será de $\min(10, 10 \cdot l/k)$ pontos, arredondada para duas casas decimais. Nesta fórmula, $l$ é o número de crianças que podem se esconder no labirinto de saída e $k$ é o número dado na entrada. Você terá 10 pontos num caso de teste se e só se o seu output é um labirinto no qual $k$ crianças podem se esconder. Para cada caso de teste é garantido que existe uma solução que consegue os 10 pontos. 
...
Em baixo está uma possível saída válida:
...
Como $l=4$ crianças se conseguem esconder no labirinto, esta solução irá receber $10 \cdot 4/5 = 8$ pontos. As células nas quais as crianças podem se esconder estão marcadas em baixo com o carácter `O`:
...
As seguintes três saídas não são válidas:
...
Na saída da esquerda não existe um caminho simples entre a célula livre do canto superior esquerdo e a célula livre da coluna mais à direita. Nas outras duas saídas, para cada par de células livres distintas existem exatamente dois caminhos simples entre elas.