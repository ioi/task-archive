...
Arezou e o seu irmão são gêmeos. Eles receberam um incrível conjunto de trens de brinquedo para o seu aniversário e usaram-no para construir um sistema de caminhos de ferro com $n$ estações e $m$ linhas *unidirecionais*. As estações estão numeradas de $0$ a $n-1$. Cada linha começa numa estação e termina na mesma ou numa estação diferente. Há pelo menos uma linha que começa em cada estação.

Algumas estações são *estações de carregamento*. Quando um trem chega a uma estação de carregamento, ele fica completamente carregado. Um trem completamente carregado tem bateria suficiente para se movimentar por $n$ linhas consecutivas. Isto é, um trem fica sem bateria quando entra na $(n+1)$-ésima linha após ter sido carregado.
...
Os gêmeos estão jogando um jogo com o seu trem. Eles já dividiram todas as estações entre si: cada estação pertence ou à Arezou ou ao Borzou. Existe apenas um único trem. No início do jogo o trem está na estação $s$ e está completamente carregado. Para iniciar o jogo, o dono da estação $s$ aponta o interruptor na estação $s$ para uma das linhas que começa na estação $s$. Em seguida, eles ligam o trem e o trem começa a andar pelas linhas.

Sempre que um trem entra numa estação pela primeira vez, o dono dessa estação escolhe a posição do interruptor nessa estação. Sempre que um interruptor é colocado numa posição, ele fica na mesma posição para o resto do jogo. Assim, se um trem reentrar numa estação que visitou antes, ele irá sair pela mesma estação que saiu anteriormente.

Visto que há um número finito de estações, o trem eventualmente entrará num *ciclo*. Um ciclo é uma sequência de estações *distintas* $c[0], c[1], \cdots, c[k-1]$ tal que o trem sai da estação $c[i]$ (para $0 \leq i < k - 1$) pela linha que termina na estação $c[i+1]$, e sai da estação $c[k-1]$ usando a linha que termina na estação $c[0]$. Note que um ciclo pode consistir de uma única estação (i.e. $k=1$) se o trem sair da estação $c[0]$ usando uma linha que volta à estação $c[0]$.

Arezou ganha o jogo se o trem continua a andar indefinidamente, e o Borzou ganha se o trem ficar sem bateria. Por outras palavras, se existir pelo menos uma estação de carregamento nas $c[0], c[1], \cdots, c[k-1]$ então o trem pode recarregar e ficar em ciclo indefinidamente, e a Arezou ganha. Caso contrário, ficará sem bateria (possivelmente após percorrer o ciclo várias vezes), e o Borzou ganha.

Será dada a descrição do sistema de caminhos de ferro. A Arezou e o Borzou jogarão $n$ jogos. No $s$-ésimo jogo, para $0 \leq s \leq n-1$, o trem estará inicialmente na estação $s$. A sua tarefa é encontrar, para cada jogo, se existe uma estratégia para a Arezou que garanta que ela ganhe, independentemente de como o Borzou jogar.
...
* Existe pelo menos uma linha começando em cada estação.
...
O corretor de exemplo lê a entrada no formato seguinte:
* linha $1$: $\;\; n \;\; m$
* linha $2$: $\;\; a[0] \;\; a[1] \; \ldots\; a[n-1]$
* linha $3$: $\;\; r[0] \;\; r[1] \; \ldots\; r[n-1]$
* linha $4 + i$ (para $0 \leq i \leq m-1$): $\;\; u[i] \;\; v[i]$

O corretor de exemplo imprime o valor de retorno de `who_wins` no formato seguinte:
