...
* para cada par de ciudades, es posible llegar de una a la otra viajando solo a través de vías de este conjunto.
...
Tu programa debe ayudar a Zal a encontrar el conjunto de vías reales haciendo a lo más $q$ preguntas a Simurgh. El calificador jugará el rol de Simurgh.
...
Considera que la función `count_common_roads` en el calificador de ejemplo no verifica si $r$ tiene todas las propiedades de un conjunto dorado. Por el contrario, cuenta y retorna la cantidad de etiquetas de vías reales en el arreglo $r$. Sin embargo, si tu programa llama a `count_common_roads` con un conjunto de etiquetas que no describe un conjunto dorado, el veredicto del CMS será ‘Wrong Answer’.
