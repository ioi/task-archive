...
O avaliador de exemplo escreve `YES` se `find_roads` chamar `count_common_roads` no máximo $30\,000$ vezes, e retorna o conjunto de estradas da realeza correto. Caso contrário, escreve `NO`.
...
A função `count_common_roads` em C++ e Pascal usa o método de *pass by reference* (passagem por referência) por razões de eficiência. É possível chamar a função da forma usual. É garantido que o avaliador não altera o valor de $r$.