...
Eres el afortunado participante que ha logrado llegar a la ronda final.
Te encuentras parado delante de una fila de $n$ cajas, numeradas de $0$ a $n-1$, de izquierda a derecha.
...
El premio de tipo $v$ es el más barato de todos: un lollipop.
...
* Entre las cajas que se encuentran a la derecha de la caja $i$, hay exactamente $a[1]$ cajas que contienen un premio más caro que el que hay en la caja $i$.
...
* Esta función es llamada una única vez por el calificador.
* $n$: cantidad de cajas.
...
* Esta función retorna el arreglo $a$ con $2$ elementos, de forma que $a[0]$ sea la cantidad de premios más caros en cajas a la izquierda de la caja $i$, y $a[1]$ sea la cantidad de premios más caros en cajas a la derecha de la caja $i$.
...
En algunos casos de prueba, el comportamiento del calificador es adaptativo.
Esto quiere decir que en estos casos de prueba, el calificador no tiene una secuencia de premios fija.
En lugar de esto, las respuestas dadas por el calificador podrían depender de las preguntas realizadas por tu solución.
Se garantiza que el calificador responderá de forma tal que luego de cada respuesta, exista al menos una secuencia de premios consistente con todas las respuestas dadas hasta el momento.

1. (20 puntos) Hay exactamente $1$ diamante y $n-1$ lollipops (o sea, $v = 2$).
...
Questions | Score
--- | ---
$10\,000 < q$ | $0$ (en CMS será 'Wrong Answer')
...
El calificador de ejemplo no es adaptativo.
...
El calificador de ejemplo lee la entrada con el siguiente formato:

- línea $1$: $\;\; n$
- línea $2$: $\;\; p[0] \;\; p[1]  \; \ldots \; p[n-1]$

El calificador de ejemplo imprime una única línea con el resultado de `find_best` y la cantidad de llamadas realizadas a la función `ask`.
