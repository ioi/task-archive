...
Pred tebe je postavljen niz od $n$ kutija, označenih od $0$ do $n-1$, slijeva udesno.
Svaka kutija sadrži nagradu koja ostaje nepoznata dok se ta kutija ne otvori.
Postoji $v \geq 2$ tipova nagrada.
Tipovi su numerisani od $1$ do $v$ po *opadajućem* redosljedu vrijednosti.
...
Preciznije, za svako $t$ za koje $2 \leq t \leq v$ znamo sljedeće:
...
* Grejder će ovu funkciju pozvati tačno jednom. 
* $n$: broj kutija.
* Ova funkcija treba da vrati oznaku kutije koja sadrži dijamant, tj. cio broj $d$ ($0 \leq d \leq n-1$) takav da kutija $d$ sadrži nagradu tipa $1$.
...
* Ova funkcija vraća niz $a$ sa $2$ elementa. Ovde je $a[0]$ broj skupljih nagrada u kutijama lijevo od kutije $i$, a $a[1]$ je broj skupljih nagrada u 
kutijama desno od kutije $i$.

## Primjer

Grejder poziva sljedeću proceduru:
...
U nekim test primjerima grejder može da se ponaša adaptivno. To znači da u tim test primjerima grejder nema fiksan niz nagrada, nego odgovor grejdera može 
da zavisi od pitanja koje mu tvoj program postavi.
...
1. (20 bodova) Postoji tačno $1$ dijamant i $n-1$ lizalica ($v = 2$). Možes pozvati proceduru `ask` najviše $10\,000$ puta.
...
Primjer grejdera ispisuje jednu liniju koja sadrži vraćenu vrijednost od funkcije `find_best` i broj poziva procedure `ask`.