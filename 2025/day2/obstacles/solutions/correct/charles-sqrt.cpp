...
struct RangeMinimumQuery {
  ...
  RangeMinimumQuery(const vector<int> &values) {
    n = (SZ(values) + BLOCK - 1) / BLOCK * BLOCK;
    v = values;
    while (SZ(v) != n)
      v.push_back(-inf);
    b.resize(n / BLOCK);
    for (const int i : to(n / BLOCK))
      b[i] = *min_element(v.begin() + i * BLOCK, v.begin() + (i + 1) * BLOCK);
  }
  ...
  int range_minimum(const int l, const int r) const {
    assert(n != -1);
    assert(0 <= l && l <= r && r <= n);
    const int lb = l / BLOCK, rb = r / BLOCK;
    if (lb == rb)
      return minr(v, l, r);
    return min({
        minr(v, l, BLOCK * (lb + 1)),
  ...
  int first_less(int i, const int val) const {
    assert(n != -1);
    const int ib = i / BLOCK;
    for (const int j : tos(i, (ib + 1) * BLOCK))
      if (v[j] < val)
        return j;
    int k = ib + 1;
    while (k < SZ(b) && b[k] >= val)
      ++k;
    for (const int j : tos(k * BLOCK, n))
      if (v[j] < val)
        return j;
    return n;
  }
  ...
  int last_less(int i, const int val) const {
    assert(n != -1);
    const int ib = i / BLOCK;
    for (const int j : froms(ib * BLOCK, i))
      if (v[j] < val)
        return j;
    int k = ib;
    while (k > 0 && b[k - 1] >= val)
      --k;
    for (const int j : froms(0, k * BLOCK))
      if (v[j] < val)
        return j;
    return -1;
  }
