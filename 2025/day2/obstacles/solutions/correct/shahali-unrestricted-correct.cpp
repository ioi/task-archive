// solution/shahali-unrestricted-correct.cpp
// {
//   "verdict": "correct",
//   "except": {
//     "samples": "runtime_error",
//     "full": "runtime_error"
//   }
// }
// END HEADER
...
int pref_maxT[MAXN];
int L[MAXN], R[MAXN];
int nexL[MAXN], nexR[MAXN], nex[MAXN];
int tmp[MAXN];
...
int dfs(int node){
	if (nex[node]) return nex[node];
	if (nexL[node]!=node) return nex[node]=dfs(nexL[node]);
	if (nexR[node]!=node) return nex[node]=dfs(nexR[node]);
	return nex[node]=node;
}
...
int go(int l, int r, int x){
	while (1){
		if (nexL[x]!=x && l<=nexL[x]) x=nexL[x];
		else if (nexR[x]!=x && nexR[x]<=r) x=nexR[x];
		else break;
	}
	return x;
}
