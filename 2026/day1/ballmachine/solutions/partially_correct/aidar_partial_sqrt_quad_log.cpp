	...
	for (int i = m - b; ; i -= b) {
		...
		for (int q = 1; q < i; q *= C) {
			for (int j = 0; j < i; j++) {
				...
				insert(j, value_d + b + c); 

				while (insert(j, value_c + b)) { }
			}
			...
			int last_d = 0;
			vector< pair<int, int> > cd(ord.size(), {-1, -1}); 
            for (int j = 0; j < (int)ord.size(); j++) {
				if (ord[j] >= b + c) {
					last_d = j; 
				} else if (ord[j] >= b) {
					int c_ = ord[j] - b;
					int d_ = ord[last_d] - b - c;
					cd[j] = cd[last_d] = {c_, d_}; 
				} 
            }
			...
			vector<int> last = { b + c + d }; 
			vector<int> last_id = { -1 };
			...
			for (int j = 0; j < (int)ord.size(); j++) {
                ...
                if (ord[j] < b && !used[ord[j]]) {
                    ...
                    if (ord[par_id] < b) {
						...
						auto [c_, d_] = cd[par_id]; 
						path_par[u] += q * (d_ * c + c_); 
                    }
                }
			}
		}
