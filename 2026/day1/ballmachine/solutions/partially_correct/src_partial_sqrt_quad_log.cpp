...
result s_1000(vector<int> l) {
  ...
  function<void()> recur = [&]() {
    int rt = i;
    i++;
    while(i < (int)ord.size() && ord[i] <= ord[rt]) {
...
vector< int > find_structure(int m) {
  ...
  // solve first b + c + d leaves directly
  vector<int> first_bcd(min(b + c + d, m));
  iota(all(first_bcd), 0);
  auto res = s_1000(first_bcd);
  for(int i = 0; i < (int)first_bcd.size(); i++) {
    path_par[i] = res.path_par[i];
    dep[i] = res.dep[i];
    path_len[i] = res.path_len[i];
  }

  for(int i = b + c + d; i < m; i += b) {
    for(int o = 1; o < i; o *= c * d) {
      for(int j = 0; j < i; j++) {
        int val = j / o;
        insert(j, b + c + val / c % d);
        while(insert(j, b + val % c));
      }
      ...
      int j = 0;
      while(j < (int)ord.size()) {
        // labels of the current path in X
        int dval = ord[j] - b - c;
        int cval;
        // paths which are a child of this path
        vector<int> children;
        function<void()> recur = [&]() {
          int rt = j;
          j++;
          while(j < (int)ord.size() && ord[j] <= ord[rt] && ord[j] < b + c) {
            if(ord[j] >= b && ord[j] < b + c) {
              cval = ord[j] - b;
            }
            if(ord[j] != ord[rt] && ord[j] < b) {
              if(ord[rt] < b) {
                ...
                children.push_back(i + b - 1 - ord[j]);
              }
            }
            recur();
          }
        ...
        for(auto child : children) {
          path_par[child] += (dval * c + cval) * o;
        }
      }
    }
  }

  // find depths in batches of size b + c + d
  for(int i = b + c + d; i < m; i += b + c + d) {
    vector<int> inds(min(b + c + d, m - i));
    iota(all(inds), i);
