 ...
 * Zsolt Németh:
 *
 * * Added 'ADMIN_MESSAGE: ' prefix to messages in quit().
 *    This allows checkers to display messages in the admin panel of CMS.
 * 
 */
...
NORETURN void InStream::quit(TResult result, const char* msg)
{
    ...
    // Add 'ADMIN_MESSAGE: ' prefix to msg to display it in the admin panel of CMS.
    if (testlibMode == _checker && strlen(msg) > 0 && msg[0] != '\n') {
        std::string adminMessagePrefix = "ADMIN_MESSAGE: ";
        std::string message(msg);
        if (message.substr(0, adminMessagePrefix.length()) != adminMessagePrefix) {
            message = adminMessagePrefix + message;
            quit(result, message.c_str());
        }
    }
